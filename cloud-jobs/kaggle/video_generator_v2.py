...
def verify_imports():
    try:
        ...
        import transformers
        import diffusers
        hf_ver = huggingface_hub.__version__
        tr_ver = transformers.__version__
        di_ver = diffusers.__version__
        print(f"✅ huggingface-hub : {hf_ver}")
        print(f"✅ transformers    : {tr_ver}")
        print(f"✅ diffusers       : {di_ver}")

        # Version check
        from packaging.version import Version
        if Version(hf_ver) >= Version("1.0"):
            print("❌ huggingface-hub बहुत नया है! Kernel restart करें")
            print("   Menu → Run → Restart & Clear Output")
            return False
        return True
